            ...
            for message in reversed(messages):
                if message.media:
                    ...
                    if isinstance(message.media, MessageMediaPhoto):
                        # Forward the photo to the destination channel with the caption
                        await self.client.send_file(destination_channel_id, message.media.photo, caption=caption)
                        print("Photo forwarded with caption")
                    elif isinstance(message.media, MessageMediaDocument):
                        # Forward the document to the destination channel with the caption
                        await self.client.send_file(destination_channel_id, message.media.document, caption=caption)
                        print("Document forwarded with caption")
                    else:
                        print("Unsupported media type detected, not forwarded")
                elif keywords:
                    ...
                    # Forward the text message to the destination channel
                    await self.client.send_message(destination_channel_id, message.text)
                    print("Message forwarded")
